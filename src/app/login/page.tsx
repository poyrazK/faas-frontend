...
/* ==========================================================================
   ...
   The backend moved to email + password (Argon2id) — this page used to post
   only an email, which the API rejected with "password is required".

   Anti-enumeration is the backend's contract and the UI has to preserve it:
   an unknown email, a wrong password and an OAuth-only account all return the
   same 401, and the forgot-password call always reports success. Nothing here
   may tell the visitor whether an address is registered.
   ========================================================================== */
...
import { googleAuthUrl, githubAuthUrl, requestPasswordReset, PASSWORD_MIN_LENGTH, ApiError } from '@/lib/api';
...
function LoginInner() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'forgot') {
        ...
        setStatus('sent');
        // Deliberately unconditional — saying "no such account" here would
        // turn this form into an email-enumeration oracle.
      ...
      if (acct) {
        ...
        // The POST succeeded but no session cookie took hold — cookies
        // blocked, or an insecure origin where the Secure cookie is dropped.
        setStatus('error');
  ...
  const heading = mode === 'signup' ? 'Create your account' : mode === 'forgot' ? 'Reset your password' : 'Sign in to Gregale';
  const blurb =
    mode === 'signup'
      ? `Pick a password of at least ${PASSWORD_MIN_LENGTH} characters. No complexity rules.`
      : mode === 'forgot'
    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-5 py-12">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={submit} className={mode === 'forgot' ? 'mt-7 space-y-3' : 'space-y-3'}>
            <div>
            ...
            <div
              ...
              style={
                status === 'error'
                  ...
                  : { background: 'var(--color-brand-soft)', color: 'var(--color-brand-bright)', border: '1px solid var(--color-brand-line)' }
              }
              role="status"
            >
              {message}
          ...
          <p className="mt-6 text-sm" style={{ color: 'var(--color-ink-muted)' }}>
            {mode === 'signin' && (
              <>
                No account?{' '}
                <button className="font-medium" style={{ color: 'var(--color-brand)' }} onClick={() => switchMode('signup')}>
                  Create one
                </button>
              </>
            )}
            {mode === 'signup' && (
              <>
                ...
                <button className="font-medium" style={{ color: 'var(--color-brand)' }} onClick={() => switchMode('signin')}>
                  Sign in
                </button>
              </>
            )}
