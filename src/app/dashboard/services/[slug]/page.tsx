...
import {
  ...
  listInstances, listSecrets, setSecret, deleteSecret, listEnv, setEnv, deleteEnv, listAppDeployments,
  listDomains, listCrons, listInvocations, getAppMetrics, isDegraded, appLogsUrl,
  listAppWebhooks, createAppWebhook, updateAppWebhook, deleteAppWebhook, rotateAppWebhookSecret,
  listAppWebhookDeliveries, retryAppWebhookDelivery, type AppWebhook,
  type AppWebhookEventType, type AppWebhookRetryPolicy,
...
const TABS = ['Overview', 'Test', 'Deployments', 'Instances', 'Logs', 'Env Vars', 'Secrets', 'Webhooks', 'Configuration', 'Domains', 'Triggers'] as const;
...
export default function ServiceDetailPage() {
  ...
  const app = useAsync(() => getApp(slug), [slug], 5000);
  const instances = useAsync(() => listInstances(slug), [slug], 3000);
  const secrets = useAsync(() => listSecrets(slug), [slug]);
  const envVars = useAsync(() => listEnv(slug), [slug]);
  const webhooks = useAsync(() => listAppWebhooks(slug), [slug]);
  const deployments = useAsync(() => listAppDeployments(slug), [slug], 5000);
  ...
  const [whRetryPolicy, setWhRetryPolicy] = useState<AppWebhookRetryPolicy>('default');
  const [whEvents, setWhEvents] = useState<AppWebhookEventType[]>([]);
  const [whActiveWebhook, setWhActiveWebhook] = useState<AppWebhook | null>(null);
  const [whDeliveryOpen, setWhDeliveryOpen] = useState(false);
  const whDeliveries = useAsync(
    () => (whActiveWebhook ? listAppWebhookDeliveries(slug, whActiveWebhook.id) : Promise.resolve(null)),
    [slug, whActiveWebhook?.id],
  );
  ...
  async function saveEnvVar(e: React.FormEvent) {
    ...
    setBusy('env');
    try {
      await setEnv(slug, envKey.trim(), envVal);
      toast.success(`Env var ${envKey.trim()} saved.`);
      setEnvOpen(false);
      setEnvKey('');
      setEnvVal('');
      envVars.reload();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : 'Could not save env variable.');
    } finally {
      setBusy(null);
    }
  }
  ...
  async function saveWebhook(e: React.FormEvent) {
    ...
    try {
      if (whEditingId) {
        await updateAppWebhook(slug, whEditingId, {
          target_url: whTargetUrl.trim(),
          event_filter: whEvents,
          retry_policy: whRetryPolicy,
          webhook_secret: whSecret.trim() || undefined,
        });
        toast.success('Webhook updated.');
      } else {
        await createAppWebhook(slug, {
          target_url: whTargetUrl.trim(),
          webhook_secret: whSecret.trim() || undefined,
          event_filter: whEvents,
          retry_policy: whRetryPolicy,
  ...
  return (
    <div>
      ...
      >
        <Icon name="chevronLeft" size={14} /> Services
      </Link>
          ...
          <>
                ...
                <SectionCard title="Endpoint">
                  <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                    ...
                    <div className="flex items-center gap-2">
                      <StatusBadge state={a.status} hasLiveInstance={(instances.data?.length ?? 0) > 0} isDeployed={(deployments.data?.items?.length ?? 0) > 0} evictionPriority={a.eviction_priority} />
            ...
            {tab === 'Env Vars' && (
              ...
              >
                <AsyncBoundary
                  state={envVars}
                  isEmpty={(d) => d.env.length === 0}
                  skeleton={<SkeletonTable cols={4} rows={3} />}
                  empty={<EmptyState icon="env" title="No environment variables" hint="Plaintext configuration injected into the guest environment at boot." />}
                >
                  {(data) => (
                    <>
                      <table className="dtable">
                        ...
                        <tbody>
                          {data.env.map((e) => (
                            <tr key={e.key}>
                              <td className="mono cell-primary">{e.key}</td>
                              <td>
                                <span className="badge badge-muted" title="Values are not returned on list">
                                  not returned
                                </span>
                              </td>
                              <td>{relativeTime(e.created_at)}</td>
                              <td>{relativeTime(e.updated_at)}</td>
                              <td className="text-right flex items-center justify-end gap-2">
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    if (!confirm(`Delete ${e.key} from ${slug}?`)) return;
                                    try {
                                      await deleteEnv(slug, e.key);
                                      toast.success(`Env var ${e.key} deleted.`);
                                      envVars.reload();
                                    } catch (err) {
                ...
                <AsyncBoundary
                  ...
                  empty={
                    <EmptyState
                      ...
                      hint="Subscribe target URLs to events emitted by this workflow (cron.fired, build.succeeded, etc.)."
                      action={
                        <button
                          className="btn btn-primary btn-sm"
                          onClick={() => {
                    ...
                    <div className="overflow-x-auto">
                      <table className="dtable">
                        ...
                        <tbody>
                          {list.map((w) => (
                            <tr key={w.id}>
                              <td className="mono cell-primary max-w-[280px] truncate" title={w.target_url}>
                                {w.target_url}
                              </td>
                              <td>
                                {w.event_filter.length === 0 ? (
                                  <span className="badge badge-muted">All events</span>
                                ) : (
                              ...
                              <td>
                                <span className="badge badge-muted">{w.retry_policy}</span>
                              </td>
                              <td>
                                {w.enabled ? (
                                  <span className="badge badge-brand">Enabled</span>
                                ) : (
                              ...
                              <td>
                                <span className="mono text-xs" style={{ color: 'var(--color-ink-muted)' }}>
                                  {w.webhook_secret_sealed_masked}
                                </span>
                              </td>
                              <td>{relativeTime(w.created_at)}</td>
                              <td>
                                <RowMenu>
                                  <RowMenuItem
                                    onClick={() => {
                                      setWhActiveWebhook(w);
                                      setWhDeliveryOpen(true);
                                    }}
                                  >
                                    View Deliveries
                                  </RowMenuItem>
                                  <RowMenuItem
                                    onClick={async () => {
                                      try {
                                        await rotateAppWebhookSecret(slug, w.id);
                                  ...
                                  <RowMenuItem
                                    onClick={async () => {
                                      try {
                                        await updateAppWebhook(slug, w.id, { enabled: !w.enabled });
                                        toast.success(`Webhook ${w.enabled ? 'disabled' : 'enabled'}.`);
                                        webhooks.reload();
                                      } catch (err) {
                                  ...
                                  <RowMenuItem
                                    danger
                                    onClick={async () => {
                                      if (!confirm(`Delete webhook subscription for ${w.target_url}?`)) return;
                                      try {
                                        await deleteAppWebhook(slug, w.id);
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          router.replace(`/dashboard/services/${renameTo}`);
                        } catch (err) {
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success(`Service ${slug} deleted.`);
      ...
      <Modal
        ...
        title={envEditing ? `Replace ${envKey}` : 'New environment variable'}
        footer={
          <>
            <button className="btn btn-secondary" onClick={() => setEnvOpen(false)}>Cancel</button>
            <button className="btn btn-primary" form="add-env-var" type="submit" disabled={busy === 'env'}>
              {busy === 'env' ? 'Saving…' : envEditing ? 'Replace value' : 'Save variable'}
            </button>
          </>
        }
      >
        <form id="add-env-var" onSubmit={saveEnvVar} className="space-y-4">
          <div>
            <label className="label">Key</label>
            <input
              className="field mono"
          ...
          <div>
            <label className="label">Value</label>
            <textarea className="field mono" rows={3} value={envVal} onChange={(e) => setEnvVal(e.target.value)} required />
            <p className="mt-1 text-xs" style={{ color: 'var(--color-ink-muted)' }}>
              Stored as plaintext configuration. Do not put credentials here — use a sealed secret instead.
            </p>
          </div>
        </form>
      </Modal>
      ...
      <Modal
        ...
        title={whEditingId ? 'Edit Outbound Webhook' : 'New Outbound Webhook'}
        footer={
          <>
            <button className="btn btn-secondary" onClick={() => setWhOpen(false)}>Cancel</button>
            <button className="btn btn-primary" form="add-webhook" type="submit" disabled={busy === 'webhook'}>
              {busy === 'webhook' ? 'Saving…' : whEditingId ? 'Save Changes' : 'Create Webhook'}
            </button>
          </>
        }
      >
        <form id="add-webhook" onSubmit={saveWebhook} className="space-y-4">
          <div>
            <label className="label">Target Endpoint URL</label>
            <input
              type="url"
              className="field mono"
              placeholder="https://api.yourdomain.com/webhooks/gregale"
              value={whTargetUrl}
              onChange={(e) => setWhTargetUrl(e.target.value)}
              required
            />
            <p className="mt-1 text-xs" style={{ color: 'var(--color-ink-muted)' }}>
              Must be a publicly reachable HTTPS URL. Loopback and internal private IP ranges are blocked.
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs" style={{ color: 'var(--color-ink-muted)' }}>
              Used to sign payload header <Mono>X-Faas-Webhook-Signature</Mono>. Sealed server-side.
            </p>
          </div>
          ...
          <div>
            ...
            <select
              className="field"
              value={whRetryPolicy}
              onChange={(e) => setWhRetryPolicy(e.target.value as AppWebhookRetryPolicy)}
            >
              <option value="default">Default (5s tick, 7 attempts over 7.5h)</option>
              <option value="aggressive">Aggressive (Faster retry backoff)</option>
              <option value="none">None (Single shot, no retries)</option>
            </select>
          </div>

          <div>
            <label className="label">Event Subscriptions</label>
            <p className="mb-2 text-xs" style={{ color: 'var(--color-ink-muted)' }}>
              Select specific events to trigger this webhook. Leave all unchecked to subscribe to all events.
            </p>
            <div className="grid grid-cols-2 gap-2">
              {(['cron.fired', 'build.succeeded', 'build.failed', 'app.created', 'app.deleted'] as AppWebhookEventType[]).map((ev) => {
                const checked = whEvents.includes(ev);
                return (
                  <label key={ev} className="flex items-center gap-2 text-xs font-medium cursor-pointer">
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={(e) => {
      ...
      {/* ── Webhook Delivery Inspector Modal ──────────────────────────────── */}
      <Modal
        open={whDeliveryOpen}
        onClose={() => setWhDeliveryOpen(false)}
        title={`Delivery History: ${whActiveWebhook?.target_url ?? ''}`}
        footer={
          <button className="btn btn-secondary" onClick={() => setWhDeliveryOpen(false)}>
            Close
          </button>
        }
      >
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs" style={{ color: 'var(--color-ink-muted)' }}>
              Recent dispatch attempts (7.5h backoff schedule, 7 retries before DLQ).
            </span>
            <button className="btn btn-ghost btn-sm" onClick={() => whDeliveries.reload()}>
              <Icon name="refresh" size={13} /> Refresh
            </button>
          </div>
          ...
          <AsyncBoundary
            state={whDeliveries}
            isEmpty={(d) => !d || d.deliveries.length === 0}
            skeleton={<SkeletonTable cols={5} rows={3} />}
            empty={
              <EmptyState
                icon="logs"
                title="No deliveries recorded"
                hint="No events have been dispatched to this webhook target URL yet."
              />
            }
          >
            {(d) => (
              <div className="overflow-x-auto max-h-[360px]">
                <table className="dtable">
                  ...
                  <tbody>
                    {d!.deliveries.map((item) => (
                      <tr key={item.id}>
                        <td className="mono text-xs font-semibold">{item.event}</td>
                        <td>
                          {item.status === 'succeeded' ? (
                            <span className="badge badge-brand">succeeded</span>
                          ) : item.status === 'dead' ? (
                            <span className="badge badge-danger">dead (DLQ)</span>
                          ) : item.status === 'failed' ? (
                        ...
                        <td>{item.attempt}/7</td>
                        <td className="max-w-[200px] truncate text-xs mono">
                          {item.last_response_code ? (
                            <span style={{ color: item.last_response_code < 400 ? 'var(--color-brand)' : 'var(--color-danger)' }}>
                              HTTP {item.last_response_code}
                            </span>
                          ) : item.last_error ? (
                            <span style={{ color: 'var(--color-danger)' }} title={item.last_error}>
                              {item.last_error}
                            </span>
                          ) : (
                            '—'
                        ...
                        <td className="text-xs">{relativeTime(item.delivered_at || item.updated_at)}</td>
                        <td className="text-right">
                          {item.status === 'dead' && (
                            <button
                              className="btn btn-primary btn-xs"
                              onClick={async () => {
                                if (!whActiveWebhook) return;
                                try {
                                  await retryAppWebhookDelivery(slug, whActiveWebhook.id, item.id);
                                  toast.success('Delivery re-armed for retry.');
                                  whDeliveries.reload();
                                } catch (err) {
                                  toast.error(err instanceof ApiError ? err.message : 'Retry trigger failed.');
                                }
                              }}
                            >
                              Retry
